...
opt = 3; % select optimality criterion
...
for i=1:length(x)
    dx(i,:) =  coupled_vdp_lorenz(t,x(i,:),mu,sigma,rho,beta,tau_fast,...
    tau_slow,c1,c2);
    noisy_dx(i,:) = coupled_vdp_lorenz(0,x(i,:),mu,sigma,rho,beta,tau_fast,...
    tau_slow,c1,c2)' + eta*rand(1,n);
end
