...
StepHandle = @(Action,LoggedSignals) StepFunction_CVDP(Action,LoggedSignals,envConstants);
% Use the same reset function, specifying it as a function handle rather than by using its name.
ResetHandle = @() ResetFunction_CVDP(NSR);
% Create the environment using the custom function handles.
...
%     plot(experience.Action.SamplingAction.Data(:))
    ...
    cx = experience.Observation.Observations.Data(5:8,:,:);
    dx = experience.Observation.Observations.Data(9:end,:,:);
    XRL = reshape(cx,4,length(cx))';
    dXRL = reshape(dx,4,length(dx))';
    % % RL
    k = 1;
    tnow = dt;
    Ts_low = 0.004;
...
%     Xtmp = [Xtmp; record(5:8,1,i)'];
%     dXtmp = [dXtmp; record(9:12,1,i)'];
