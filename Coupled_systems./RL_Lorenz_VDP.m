...
dx = zeros(length(x),n);
for i = 1:length(x)
    dx(i,:) = coupled_vdp_lorenz(0,x(i,:),par_vec);
end
...
envConstants.c1 = 0.01; 
envConstants.c2 = 10;
envConstants.sigma = 10;
envConstants.rho = 28;
envConstants.beta = 8/3;
...
StepHandle = @(Action,LoggedSignals) LorenzVDPStepFunction(Action,LoggedSignals,envConstants);
% Use the same reset function, specifying it as a function handle rather than by using its name.
ResetHandle = @() LorenzVDPResetFunction(envConstants);
% Create the environment using the custom function handles.
