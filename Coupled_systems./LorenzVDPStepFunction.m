function [NextObs,Reward,IsDone,LoggedSignals] =...
    LorenzVDPStepFunction(Action,LoggedSignals,envConstants)
...
[~,x_next] = ode45(@(t,x) coupled_vdp_lorenz(t,x,par_vec),tspan,state,options);
for i = 1:n
    LoggedSignals.State(i) = x_next(2,i) + envConstants.eta*rand(); %% Add noise to the state
end
noisy_dx_next =  coupled_vdp_lorenz(0,x_next(2,:),par_vec)' + envConstants.eta*rand(1,n);
...
Xi_hat = sparsifyDynamics(Theta,x(:,(n+1):2*n),envConstants.lambda,n);
...
acf_t = autocorr(x(:,4));
acf_u = autocorr(x(:,5));
MI = abs(mean([acf_x(2) acf_y(2) acf_z(2) acf_t(2) acf_u(2)]));
